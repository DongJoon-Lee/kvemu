...
void ssd_advance_write_pointer(struct ssd *ssd, struct line_partition *lm)
{
    ...
    if (wpp->pg % flash_types == (flash_types-1)) { /* When completed writing data for all cell types */
    ...
    } else if (wpp->pg +1 == spp->pgs_per_blk) { /* When size of cell type range differs from pgs_per_blk */
        wpp->pg = spp->pgs_per_blk/flash_types*flash_types;
        wpp->ch++;
        if (wpp->ch == spp->nchs) {
            wpp->ch = 0;
            check_addr(wpp->lun, spp->luns_per_ch);
